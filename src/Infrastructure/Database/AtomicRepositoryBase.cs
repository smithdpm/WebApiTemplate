...
public class AtomicRepositoryBase<T>: RepositoryBase<T>
    where T : class
{
    public AtomicRepositoryBase(DbContext dbContext)
        : this(dbContext, Ardalis.Specification.EntityFrameworkCore.SpecificationEvaluator.Default)
    {
    }

    public AtomicRepositoryBase(DbContext dbContext, ISpecificationEvaluator specificationEvaluator)
